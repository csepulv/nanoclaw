...
function buildVolumeMounts(
  ...
  if (isMain) {
    ...
    // Apple Container only supports directory mounts, not file mounts like
    // Docker's /dev/null overlay, so the entrypoint starts as root and shadows
    // .env before dropping privileges.
...
export async function runContainerAgent(
  ...
  const containerArgs = buildContainerArgs(mounts, containerName, isMain);
